...
const OverviewModule = {
  ...
  async _volumeStats(){
    try{ const{data}=await sb.from('deposits').select('amount,coin,status').eq('status','approved'); const rows=data||[]; const btc=rows.filter(r=>r.coin==='btc').reduce((s,r)=>s+Number(r.amount||0),0); const usdt=rows.filter(r=>r.coin==='usdt_bep20').reduce((s,r)=>s+Number(r.amount||0),0); setText('#stat-btc-volume',btc.toFixed(6)+' BTC'); setText('#stat-usdt-volume',usdt.toFixed(2)+' USDT'); }catch(err){console.warn(err);}
  },
  async _activityFeed(){
...
const DepositsModule = {
  ...
  async _creditBalance(depositId,dep){
    ...
    const isUSDT=dep.coin==='usdt_bep20'; const field=isUSDT?'usdt_balance':'btc_balance';
    ...
    const _createTx=async()=>{ const{error:txErr}=await sb.from('transactions').insert({user_id:dep.user_id,type:'deposit',amount:Number(dep.amount||0),coin:dep.coin||(isUSDT?'usdt_bep20':'btc'),status:'success',created_at:new Date().toISOString()}); if(txErr)throw txErr; };
    const rpcName=isUSDT?'increment_user_usdt_balance':'increment_user_balance';
    const{error:rpcErr}=await sb.rpc(rpcName,{p_user_id:dep.user_id,p_amount:dep.amount});
    if(!rpcErr){ await _createTx(); return; }
    try{
  ...
  openCreateModal(){
    ...
    setHTML(body,`
      ...
      <div class="form-group"><label>Coin</label><select id="mdCoin"><option value="btc">BTC</option><option value="usdt_bep20">USDT BEP20</option></select></div>
  ...
  async createManual(){
    ...
    const{data:dep,error}=await sb.from('deposits').insert({user_id:prof.id,user_email:email,coin,amount,tx_hash:txHash,status:'approved',created_at:new Date().toISOString()}).select().single();
...
const WithdrawalsModule = {
  ...
  async _debitBalance(row){
    ...
    const isUSDT=row.coin==='usdt_bep20'; const field=isUSDT?'usdt_balance':'btc_balance';
  ...
  openCreateModal(){
    ...
    setHTML(body,`
      ...
      <div class="form-group"><label>Coin</label><select id="mwCoin"><option value="btc">BTC</option><option value="usdt_bep20">USDT BEP20</option></select></div>
  ...
  async createManual(){
    ...
    await sb.from('transactions').insert({user_id:prof.id,type:'withdrawal',amount,coin:coin||'btc',status:'success',created_at:new Date().toISOString()});
...
const UsersModule = {
    ...
    setHTML(body,`
      <div class="grid-2" style="margin-bottom:20px;">
        ...
        <div class="card"><div class="card-title">💰 Financial Stats</div>
          <div style="font-size:13px;color:#94a3b8;line-height:1.8;">
            <div><strong style="color:#f1f5f9;">BTC Balance:</strong> ${Number(u.btc_balance||0).toFixed(8)}</div>
            <div><strong style="color:#f1f5f9;">USDT Balance:</strong> ${Number(u.usdt_balance||0).toFixed(2)}</div>
            <div><strong style="color:#f1f5f9;">Total Deposited:</strong> ${totalDep.toFixed(8)} BTC eq</div>
            <div><strong style="color:#f1f5f9;">Total Withdrawn:</strong> ${totalWit.toFixed(8)} BTC eq</div>
            <div><strong style="color:#f1f5f9;">Mining Income:</strong> ${mining.toFixed(8)} BTC</div>
            <div><strong style="color:#f1f5f9;">Referral Earnings:</strong> ${refEarn.toFixed(8)} BTC eq</div>
            <div><strong style="color:#f1f5f9;">Active Contracts:</strong> ${activeContracts.length} · ${totalHash.toFixed(1)} TH/s</div>
            <div><strong style="color:#f1f5f9;">Daily Profit:</strong> ${dailyProfit.toFixed(8)} BTC</div>
          </div>
        </div>
      </div>
  ...
  async editBalance(userId){
    const u=this._rows.find(r=>r.id===userId); if(!u)return;
    const newBtc=prompt('New BTC balance (current: '+u.btc_balance+')', u.btc_balance||0); if(newBtc===null)return;
    const newUsdt=prompt('New USDT balance (current: '+u.usdt_balance+')', u.usdt_balance||0); if(newUsdt===null)return;
    const old={btc_balance:u.btc_balance,usdt_balance:u.usdt_balance};
    const patch={btc_balance:parseFloat(newBtc)||0,usdt_balance:parseFloat(newUsdt)||0};
    const{error}=await sb.from('profiles').update(patch).eq('id',userId);
    if(error){ AdminUI.toast('Failed: '+error.message,'error'); return; }
    await logAdminAction('edit_balance','profiles',userId,old,patch);
    u.btc_balance=patch.btc_balance; u.usdt_balance=patch.usdt_balance; this._renderPage();
  ...
  export(){
    ...
    const rows=this._rows.map(r=>[r.id,r.email||'',r.name||'',r.btc_balance||0,r.usdt_balance||0,r.level||'',r.is_active!==false?'Yes':'No',r.is_admin?'Yes':'No',r.is_banned?'Yes':'No',r.is_suspended?'Yes':'No',r.ref_code||'',r.created_at||'']);
