...
const AdminAuth = {
  ...
  async login(email,password){ if(!sb || !this._roleAuth)throw new Error('Supabase client not ready.'); const deviceId=localStorage.getItem('cv_auth_device_id')||'admin-browser'; const rlUrl=`${window.CRYPTOVAULT_SUPABASE_URL}/functions/v1/auth-rate-limit`; const precheckRes=await fetch(rlUrl,{method:'POST',headers:{'Content-Type':'application/json','apikey':window.CRYPTOVAULT_SUPABASE_KEY},body:JSON.stringify({action:'precheck',email:String(email||'').toLowerCase(),device_id:deviceId})}); const precheck=await precheckRes.json().catch(()=>({})); if(!precheckRes.ok)throw new Error(precheck.error||'Rate limit service unavailable.'); if(precheck && precheck.allowed===false)throw new Error(`Too many attempts. Try again in ${Math.max(1,Number(precheck.retry_after_seconds||0))}s.`); const{data,error}=await sb.auth.signInWithPassword({email,password}); if(error){ await fetch(rlUrl,{method:'POST',headers:{'Content-Type':'application/json','apikey':window.CRYPTOVAULT_SUPABASE_KEY},body:JSON.stringify({action:'record',email:String(email||'').toLowerCase(),device_id:deviceId,success:false})}); throw new Error(error.message);} if(!data?.user?.email_confirmed_at){ await sb.auth.signOut().catch(()=>{}); throw new Error('Please verify your email before logging in.'); } const role = await this._roleAuth.resolveUserRole(sb, data.user); if(role!=='admin'){await sb.auth.signOut().catch(()=>{}); this._roleAuth.clearRole(); throw new Error('Access denied.');} await fetch(rlUrl,{method:'POST',headers:{'Content-Type':'application/json','apikey':window.CRYPTOVAULT_SUPABASE_KEY},body:JSON.stringify({action:'record',email:String(email||'').toLowerCase(),device_id:deviceId,success:true})}); this._roleAuth.touchAuthActivity?.(); this.user=data.user; this._fillUI(data.user); return data.user; },
